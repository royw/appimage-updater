...
class ValidationHelper:
    ...
    def check_download_directory_warning(self, download_dir: Path | None) -> bool:
        ...
        if not download_dir.is_dir():
            self.console.print(
                f"[bold red]Error:[/bold red] Download path '{download_dir}' "
    ...
    def validate_symlink_path(self, symlink_path: Path | None) -> list[str]:
        """Validate symlink path configuration.
        ...
        if not symlink_path.parent.exists():
            errors.append(
                f"Symlink parent directory '{symlink_path.parent}' does not exist"
            )
            
        # Check if parent is writable
        elif not symlink_path.parent.is_dir():
            errors.append(
                f"Symlink parent path '{symlink_path.parent}' is not a directory"
            )
        ...
        # Check if symlink already exists and points elsewhere
            ...
            if not target.exists():
                errors.append(
                    f"Existing symlink '{symlink_path}' points to non-existent target '{target}'"
                )
